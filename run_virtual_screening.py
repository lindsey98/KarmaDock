...
from utils.post_processing import correct_pos, correct_one
...
@torch.inference_mode()
def post_processing(lig_pos, data, mdn_score_pred, out_dir, out_corrected=True, addHs=True):
    # # post processing
    ...
    for idx, mol in enumerate(data['ligand'].mol):
        ...
        poses.append([ff_corrected_mol.GetConformer().GetPositions(), align_corrected_mol.GetConformer().GetPositions()])
        ...
        if out_corrected:
            ...
            try:
                ...
                print(f'save {align_corrected_file} failed')
                pass
...
if __name__ == '__main__':
    ...
    '''Filter results'''
    # for proteinName in os.listdir(args.proteinDirsUni)[4:]:
    ...
    #
    #     average_df_score = all_trials_data.groupby(['protein_name', 'compound_name', 'pocket_name'])[
    #         'binding_strength'].mean().reset_index()
    #
